...
\end{align*}
\type{Propag} and \type{Couple} is overloaded to give more sophisticated axonal propagation and synaptic plasticity, respectively.
...
ALL classes (with the single exception being \type{Configf}) should derive from the \type{NF} object. This abstract base class contains 3 member variables, and 4 interface methods:
...
\type{main()}& $\Rightarrow$ &Initialize the config file and dump file;\\[6pt]
&&\type{Solver::init()};\type{Solver::solve()};\\[6pt]
\type{Solver::init()}& $\Rightarrow$ &read in global parameters; Read in \type{CntMat};\\[6pt]
&&Constructs \type{Population}; constructs \type{Propag}; constructs \type{Couple}; add \type{Dendrite} to \type{Population};\\[6pt]
&&Reads configurations for \type{Population}, \type{Propag}, \type{Couple}, and \type{Output}.\\[6pt]
...
\begin{description}
\item[Array] An array container class, see Sec.~\ref{sec:array} for detail.
\item[CaDP] A coupling class implementing calcium dependent plasticity.
\item[Solver::CntMat] Reads the connectivity matrix information for initialization.
\item[Configf] A helper class inheriting from an input stream. The important functions are:
	\begin{enumerate}
	...
	\end{enumerate}
\item[Couple] Models synaptic coupling by managing \(\nu_{ab}\). This class gives constant \(nu\), but by deriving from this class you can have synaptic plasticity.
...
\item[NF] An abstract base class which ALL classes should inherit. See Sec.~\ref{sec:nf} for detail.
\item[Population] Models a neural population. If it receives any input from other populations, it is a normal population, and calculates its response via QResponse and Dendrite. Otherwise it is a stimulus response, and its firing rate is given by a Timeseries.
...
\item[Restartf] Identical to Configf, but used in restart mode.
...
\subsection{TODO}
\begin{enumerate}
\item optional initial V and phi?
\item check numerical entry of connection matrix
\item extend outputting routine
