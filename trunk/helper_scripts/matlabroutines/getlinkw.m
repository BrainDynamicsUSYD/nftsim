function dummy=getlinkw(fname)
%function dummy=getlinkw(fname)
%Read linear model parameters from eegcode.conf file for EIRS model
%and plot the k omega spectrum
...
% create k array
%
k=[0:1.0/(xlen*deltax):1/(2*deltax)];
%
% Call phiephin
%
transferfn = phiephinkw(gamma, Gee, Gei, Ges, Gse, Gre, Gsr, Grs, Gsn, t0, alpha, beta, omega,k,re);
%
% Plot the transfer function
%
contour(omega/(2*pi),k,abs(transferfn').^2,logcontour(transferfn,1))
%mesh(omega/(2*pi),k,abs(transferfn').^2)
