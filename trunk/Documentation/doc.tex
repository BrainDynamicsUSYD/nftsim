...
\item It is also possible to vary more than one parameter. For example, if you want to launch NeuroField 4 times, with parameters varying according to the following table:

\begin{tabular}{l l l}
Object(s)&Parameter&Value\\
`Propagator 1', `Propagator 2'&gamma&10, 20, 30, 40\\
`Coupling data 1'&Nu&1, 2, 3, 4
\end{tabular}
...
\end{lstlisting}
The launcher script support arbitrary numbers of objects, parameters and runs. The only restriction is that the number of sweeps must be constant across different parameters, i.e. you can't have gamma = 10, 20, 30, 40 while Nu = 1, 2, 3.

\end{enumerate}

\item To compile NeuroField without running, run \type{make}. NeuroField and object files are produced in \type{./Release/}.

\item \type{make clean} removes the entire \type{./Release/} folder, so make sure you do not put any user data in there! 

\item Documentation can be found in \type{./Documentation/} folder. To generate doc.pdf, run \type{make Documentation/doc.pdf}. \type{make clean} deletes the files created by latex.
