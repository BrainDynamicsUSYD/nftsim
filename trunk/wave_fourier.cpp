...
WaveFourier::WaveFourier( int nodes, double deltat, int index, Population& prepop,
        Population& postpop, int longside, string topology )
    : Wave( nodes, deltat, index, prepop,
        postpop, longside, topology )
{
}
...
void WaveFourier::init(Configf& configf)
{
    ...
    fftplan = fftw_plan_dft_2d(sqrt(nodes),sqrt(nodes),reinterpret_cast<fftw_complex*>(&in[0]),reinterpret_cast<fftw_complex*>(&out[0]),FFTW_FORWARD,FFTW_MEASURE);
