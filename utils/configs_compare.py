...
OPTIONS:
   ...
   --trace='trace.name' : Name of the trace to look at.
   --node=0             : Node to look at (0-based indexing).
   --diff               : Switch to enable plot type diff.
   --overlay            : Switch to enable plot type overlay.
   --help               : print a help message (autogenerated by argparse).

EXAMPLES:
  #The default configuration file to compare is onepop.conf, the default trace
  ...
  #node, with the default for other options, run:
  ./utils/configs_compare.py --conf=example.conf --node=72 --overlay --diff

  #Or just the diff for 'eirs-bg.conf', from the 'configs/Published/van-Albada_2009/'
  #directory, looking at trace 'Pop.3.V', with the default for other options, run:
  python -i ./utils/configs_compare.py --conf=eirs-bg.conf --trace='pop.3.v' --diff
