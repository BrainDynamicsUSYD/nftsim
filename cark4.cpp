...
void CaRK4::CaDE::rhs( const vector<double>& y, vector<double>& dydt )
{
  // y == { binding, H, Ca, nu }
  // binding, leave alone
  dydt[0] = 0;
...
double CaRK4::CaDE::sig( double x, double beta ) const
{
  return 1/(1+exp(-beta*x));
}

double CaRK4::CaDE::po(double Ca) const
{
  return th +ltp*sig(Ca-pth,4e7);
}

double CaRK4::CaDE::de(double Ca) const
{
  return th +ltd*sig(Ca-dth,4e7) -ltd*sig(Ca-pth,4e7);
}

void CaRK4::init( Configf& configf )
{
...
void CaRK4::restart( Restartf& restartf )
{
}

void CaRK4::dump( Dumpf& dumpf ) const
{
}

CaRK4::CaRK4( int nodes, double deltat, int index, const vector<double>& glu,
...
const vector<double>& CaRK4::nu(void) const
{
  return rk4[3];
}

vector<Output*> CaRK4::output(void) const
{
  vector<Output*> temp;
